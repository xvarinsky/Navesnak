...
class FilterConfig:
    ...
    def __init__(self, name: str, image_path: str, anchor: str, 
                 scale_factor: float = 1.0, offset_x: float = 0.0, 
                 offset_y: float = 0.0, rotation: float = 0.0):
        """
    ...
    def load_image(self) -> bool:
        ...
        try:
            ...
            if img.shape[2] == 4:
                ...
                self._image = img
                self._image_alpha = np.ones((img.shape[0], img.shape[1]), dtype=np.uint8) * 255
...
def compute_anchor_points(landmarks, frame_width: int, frame_height: int) -> Dict[str, Tuple[int, int]]:
    """
    ...
    # forehead center (10), mouth center (13)
...
def overlay_filter(frame: np.ndarray, filter_config: FilterConfig, 
    ...
    if len(roi.shape) == 3 and len(filter_cropped.shape) == 3:
        for c in range(3):
            roi[:, :, c] = (alpha_cropped * filter_cropped[:, :, c] + 
